...
/* Textarea */
textarea {
    font-size:1.05rem !important;line-height:1.4 !important;padding:14px !important;
    background-color:#fff6e9 !important;color:#7a4d00 !important;
    border-radius:14px !important;border:2px solid #ffaf52 !important;box-shadow:none !important;
...
/* Action Buttons */
.action-btn {background:linear-gradient(90deg,#ff7d00,#ffb338);border-radius:18px;padding:14px 0;color:#522f00;font-weight:700;font-size:1.2rem;border:none;width:100%;margin-top:18px;cursor:pointer;transition:background 0.3s ease, box-shadow 0.3s ease;box-shadow:0 7px 22px #ffa53bcc;}
...
/* Output card */
.output-card {background:#fff9f0;border-radius:20px;box-shadow:0 16px 48px #ff9b2b5e;padding:24px 20px 20px;font-size:1rem;color:#7a4d00;margin-bottom:28px;line-height:1.5;}
...
.copy-btn {background:#ff7f23;color:white;border:none;padding:5px 12px;border-radius:8px;font-size:0.95rem;cursor:pointer;transition:all 0.2s;}
.copy-btn:hover {background:#e56e00;}

/* Responsive */
@media(max-width:720px){.container{margin:24px;padding:28px 24px 24px;}}
</style>
...
# ---------- Function to show output with copy ----------
def output_with_copy(title, text):
    if text:
        component_code = f"""
        <div class="output-card">
            <div class="output-title">{title}</div>
            <textarea id="{title}" readonly style="width
