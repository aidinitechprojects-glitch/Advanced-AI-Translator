...
st.markdown("""
<style>
body, [data-testid="stAppViewContainer"] {background: linear-gradient(135deg, #fff8f1 0%, #ffe5d1 100%); font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;}
.container {background:#fffaf3; border-radius:18px; padding:28px 32px 24px; max-width:680px; margin:32px auto 48px; box-shadow:0 10px 35px rgba(255,135,0,0.35); border:1.6px solid #ffd8a0;}
.header{display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;}
.header-title{font-size:2.4rem; font-weight:800; color:#e66c02; letter-spacing:.03em;}
.header-badge{background-color:#ff7f23; color:white; font-weight:700; padding:5px 22px; font-size:1rem; border-radius:14px; box-shadow:0 0 18px #ffB84eaa;}
.subtitle{margin-top:4px; margin-bottom:28px; font-weight:600; font-size:1.1rem; color:#a15303cc;}
...
textarea {font-size:1rem !important; line-height:1.4 !important; padding:14px !important; background-color:#fff6e9 !important; color:#7a4d00 !important; border-radius:14px !important; border:2px solid #ffaf52 !important; min-height:90px !important; resize:vertical !important;}
...
.swap-btn{width:40px; height:40px; background:#ff7f23; font-size:24px; color:white; border-radius:50%; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; margin:auto;}
.swap-btn:hover{transform:scale(1.15);}
.action-btn{background: linear-gradient(90deg,#ff7d00,#ffb338); border-radius:14px; padding:14px 0; color:#522f00 !important; font-weight:700; font-size:1.14rem; border:none; width:100%; margin-top:18px; cursor:pointer;}
...
with col_swap:
    if st.button("⇆", key="swap"): st.session_state.source_lang, st.session_state.target_lang = st.session_state.target_lang, st.session_state.source_lang; st.experimental_rerun()
with col3:
...
with clear_col:
    if st.button("Clear", key="clear", use_container_width=True):
        st.session_state.text_input = ""; st.session_state.translated_text = ""; st.session_state.phonetic_text = ""; st.experimental_rerun()
with translate_col:
