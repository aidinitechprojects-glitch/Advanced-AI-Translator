...
if translate_clicked:
    if not st.session_state.text_input.strip():
        ...
        with st.spinner("Translating…"):
            translate_prompt = f"Translate the following text from {st.session_state.source_lang} to {st.session_state.target_lang}. ONLY provide the translated text without any explanations:\n\n{st.session_state.text_input}"
            ...
            phonetic_prompt = f"Provide phonetic (romanized) transcription of this {st.session_state.target_lang} text, raw output only:\n{st.session_state.translated_text}"
...
# ---------- Output Function with Copy ----------
def output_with_copy(title, text, key_suffix):
    if text:
        container = f'<div class="output-card"><div class="output-title">{title}</div>{text}</div>'
        st.markdown(container, unsafe_allow
